'use client';

import React from 'react';
import { motion } from 'framer-motion';
import Link from 'next/link';

export default function ProductCard({ product }) {
  const whatsappNumber = "9840031124";
  const message = `Hi I am interested in ${product.name}`;
  const whatsappUrl = `https://wa.me/91${whatsappNumber}?text=${encodeURIComponent(message)}`;

  return (
    <div className="w-full max-w-[320px] mx-auto">
      <motion.div
        initial={{ opacity: 0, y: 20 }}
        whileInView={{ opacity: 1, y: 0 }}
        whileHover={{ 
          y: -8, 
          boxShadow: "0 25px 50px rgba(0,0,0,0.08)",
          borderColor: "rgba(7,27,52,0.15)"
        }}
        viewport={{ once: true }}
        transition={{ duration: 0.45, ease: [0.22, 1, 0.36, 1] }}
        className="bg-white rounded-[28px] border border-[rgba(7,27,52,0.05)] shadow-[0_10px_30px_rgba(0,0,0,0.04)] overflow-hidden flex flex-col h-full group"
      >
        {/* Unified Image Area */}
        <Link href={`/collection/${(product.category || 'general').toLowerCase()}/${product.slug || product.id}`} className="block relative aspect-square w-full bg-[#F8F8F8] overflow-hidden">
          <img
            src={product.image_url}
            alt={product.name}
            className="w-full h-full object-cover object-center block"
          />
        </Link>

        {/* Product Info */}
        <div className="flex flex-col gap-[14px] p-6 flex-1">
          <div className="flex flex-col gap-1.5">
            <span className="text-[9px] text-[rgba(7,27,52,0.45)] font-black uppercase tracking-[0.3em]">{product.category}</span>
            <Link href={`/collection/${(product.category || 'general').toLowerCase()}/${product.slug || product.id}`} className="block">
              <h3 className="text-[17px] font-bold text-[#071B34] font-serif leading-tight line-clamp-2 hover:text-[#0C2D57] transition-colors">
                {product.name}
              </h3>
            </Link>
          </div>

          <div className="flex items-center justify-between mt-auto pt-2 border-t border-[rgba(7,27,52,0.08)]">
            <span className="text-[18px] font-black text-[#071B34] tracking-tight">₹ {Number(product.price).toLocaleString('en-IN')}</span>
            
            <a
              href={whatsappUrl}
              target="_blank"
              rel="noopener noreferrer"
              className="px-5 py-2.5 bg-[#071B34] text-white rounded-xl font-black uppercase tracking-widest text-[9px] flex items-center gap-2 hover:bg-[#0C2D57] hover:-translate-y-[2px] transition-all duration-300 shadow-sm active:scale-95"
            >
              <svg viewBox="0 0 24 24" width="12" height="12" fill="currentColor">
                <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413Z" />
              </svg>
              Enquire
            </a>
          </div>
        </div>
      </motion.div>
    </div>
  );
}
